...
### September 9, 2026 — selected Search Loads result identity

- Production worker inspection and form staging were authorized by the user, including reconnecting the shared DAT session. No SEARCH was pressed during these observations.
- On opening Search Loads, the saved search form was visible before its data finished loading: `mat-progress-bar[role="progressbar"]` was visible and `[data-test="results-counter"]` contained `Results` without a numeric count. Staging must wait for this initial hydration to finish.
- The active submitted search is represented independently by `[data-test="search-tab-group"] [role="tab"][aria-selected="true"]`. Inside that tab, `[data-test="search-tab-summary-labels"] .load-details > div` contains exactly two ordered labels: origin and destination.
- The previous saved search used a different origin from the requested shipment. After staging the requested origin and destination in the form, the active tab labels and result counter still represented that previous search. Form values alone therefore cannot establish result identity.
- The known loading indicator and numeric result counter provide completion evidence; the active tab labels provide separate lane evidence. Fail closed if the identity or observed loading generation changes while extracting results.
- A subsequent no-submit check showed that this DAT layout omits the old Include Similar Results switch. It instead displays a numeric `[data-test="results-counter"]` and a separate numeric `[data-test="similar-results-counter"]` with a leading plus sign and the label Similar Results. Only the verified direct count and rows before the observed Similar separator belong in CRM output. If a switch exists, it must still be uniquely off.
- Only relevant form values and DOM structure were inspected. No contact details, credentials, storage state, screenshots of commercial rows, or raw exports were retained in this repository.
