...
import { URL } from 'url';
import { getGoogleOAuthCredentials, GoogleOAuthRole } from '../services/googleOAuthCredentials';

const args = process.argv.slice(2);
const roleArg = args.find(function(value) {
  return value === '--send' || value === '--inbox' || value.startsWith('--role=');
});
const role: GoogleOAuthRole = (function() {
  if (roleArg === '--send') return 'send';
  if (roleArg === '--inbox') return 'inbox';
  if (roleArg && roleArg.startsWith('--role=')) {
    const value = roleArg.slice('--role='.length);
    if (value === 'send' || value === 'inbox') return value;
  }
  return 'inbox';
})();
...
const mailbox = role === 'send'
  ? (process.env.GMAIL_SEND_ACCOUNT || process.env.GMAIL_SEND_FROM_ADDRESS || '')
  : (process.env.GMAIL_QUOTE_MAILBOX || 'emailbot@optimation.io');
const refreshTokenEnvKey = role === 'send' ? 'GMAIL_SEND_REFRESH_TOKEN' : 'GMAIL_REFRESH_TOKEN';
const scopes = role === 'send'
...
if (!clientId || !clientSecret) {
  ...
  if (role === 'send') {
    console.error('Set GMAIL_SEND_CLIENT_ID and GMAIL_SEND_CLIENT_SECRET in server/.env,');
    console.error('or leave them blank to reuse GMAIL_CLIENT_ID/GMAIL_CLIENT_SECRET.');
  } else {
...
function saveRefreshToken(refreshToken: string): string {
  ...
  const line = `${refreshTokenEnvKey}=${refreshToken}`;
  const existing = fs.existsSync(envPath) ? fs.readFileSync(envPath, 'utf8') : '';
  const pattern = new RegExp(`^${refreshTokenEnvKey}=.*$`, 'm');
  const updated = pattern.test(existing)
    ? existing.replace(pattern, function() { return line; })
...
if (mailbox) authorizeUrl.searchParams.set('login_hint', mailbox);
authorizeUrl.searchParams.set('state', state);

console.log('');
if (role === 'send') {
  console.log(`Authorize the Gmail send-as account${mailbox ? ` as ${mailbox}` : ''}:`);
  console.log('(Sign in as the Gmail account that owns the send-as alias, not the alias itself.)');
} else {
...
const server = http.createServer(async function(req, res) {
  ...
  try {
    ...
    const label = role === 'send' ? 'Gmail send-as account connected' : 'Gmail quote inbox connected';
    res.end(`<h1>${label}</h1><p>You can close this window and return to the terminal.</p>`);
    const envPath = saveRefreshToken(tokens.refresh_token);
    console.log('');
    console.log(`Authorization complete. ${refreshTokenEnvKey} was saved to ${envPath}.`);
