...
async function parseCalculateRateWithOpenRouter(inputText: string): Promise<any> {
  ...
  const prompt = [
    'You extract LTL/FTL shipping quote details from a free-form user description.',
    ...
    '- Always treat the first location mentioned as the pickup (origin) and the second as the delivery (destination). Never swap them.',
    '- For pickup and delivery, you MUST provide either a ZIP code, a city+state, or both.',
    '- If a full street address is provided, extract the city, state, and ZIP from that address.',
    '- Never leave pickup.zip and delivery.zip empty when a ZIP code can be inferred.',
    '',
    'PIECES / PALLETS RULES:',
    '- The user often describes pallets or pieces. For each distinct pallet/piece configuration, create a part in pieces.parts.',
    '- If the user specifies a quantity (e.g., "2 pallets", "3 crates"), set the "count" field to that number for that part.',
    '- Example: "2 pallets @ 43.3x48x64.9 in each, 1200 lbs each" should have count: 2, length: 43.3, width: 48, height: 64.9, weight: 1200.',
    '- If no quantity is specified, default count to 1.',
    '- Use numeric values for length, width, height, and weight.',
    '- If the user gives weights in kilograms (kg), CONVERT them to pounds (lbs) before returning JSON. Use 1 kg = 2.20462 lbs and round to a reasonable number of decimals.',
    '',
    'AIRPORT CODES & AIRPORT LOCATIONS:',
    '- If the user mentions an airport (e.g., IAH, LAX, JFK, or "ship to Dallas airport"), you MUST resolve it to the airport\'s city, state, and ZIP.',
    '- Always use the ZIP code of the airport location for pickup.zip or delivery.zip (do NOT use the airport code itself as zip).',
    ...
    '',
    'GENERAL:',
    '- Be careful not to confuse pickup and delivery; keep origin/destination consistent with the user\'s wording.',
    '- If any field is truly unknown, omit it or leave it empty rather than guessing wildly.',
    '',
    'USER INPUT:',
    inputText
  ].join('\n');
